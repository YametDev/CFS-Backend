module.exports.templateEmail = (val) => {
  const rating = `<h1>${'★'.repeat(val.rating)}${'☆'.repeat(5 - val.rating)}</h1>`
  const review = val.review ? `<p>"${val.review}"</p>` : ''
  const text = `<p><b>Comment:</b></p><p>${val.review_text.length ? val.review_text : 'None'}</p>`

  const subtitle = '<br/><p><b>Reactions to Service Parts:</b></p>'
  const faces = ['😐', '🙁', '😐', '🙂']
  const wait = `<p><span style="font-size: 18px;">${
    faces[val.review_score.wait]
  }</span> Wait Time</p>`
